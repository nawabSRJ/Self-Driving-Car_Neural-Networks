...
const traffic=[
    ...
    new Car(road.getLaneCenter(1),-1030,30,50,"DUMMY",2),
]
...
function animate(time) {
    ...
    bestCar = cars.find(
        c=>c.y==Math.min(...cars.map(c=>c.y))   // make an array from y values of car and then take out the min y
    );
    ...
    road.draw(carCtx);
    for(let i = 0; i < traffic.length; i++){
        traffic[i].draw(carCtx,"red"); // traffic cars
    }
    ...
    bestCar.draw(carCtx,"blue",true); // our car  
    carCtx.restore();
    
    networkCtx.lineDashOffset = -time / 50;
    Visualizer.drawNetwork(networkCtx,bestCar.brain);    // stops the car and NO visuals of Neural Network
